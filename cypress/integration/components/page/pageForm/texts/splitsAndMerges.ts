...
describe("TextEdit splits and merges", () => {
  it("should correctly add a new paragraph", () => {
    ...
    cy.get(firstParagraph).nextAll("p").first().should("be.empty")
    cy.get(firstParagraph).nextAll("p").first().type("A new paragraph!")
    cy.get(firstParagraph).nextAll("p").first().should("have.text", "A new paragraph!")
  ...
  it("should correctly add a new paragraph with the text on right of cursor", () => {
    ...
    cy.get(firstParagraph).type("{enter}")
    cy.get(firstParagraph).nextAll("p").first().should("have.text", "text.")
    cy.get(firstParagraph).nextAll("p").first().type("Changed ")
    cy.get(firstParagraph).nextAll("p").first().should("have.text", "Changed text.")
    cy.get(secondParagraph).should("have.text", "Class aptent taciti sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos.")
  })
